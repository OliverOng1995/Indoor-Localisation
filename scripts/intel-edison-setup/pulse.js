express = require('express'); //Import the express dependency
app = express();
app.use(express.urlencoded({ extended: true })); //url encoded form
port=3000;
noble = require('noble');
zmq = require('zeromq');
...
sensor = zmq.socket('push');
sensor.setsockopt(zmq.ZMQ_SNDHWM, 2000);
sensor.setsockopt(zmq.ZMQ_TCP_KEEPALIVE, 1);
sensor.connect(config.zmqSockets.airsensor.pushpull);
...
function sendSensorData(air_quality,humidity,temperature,light,CO2,sensorID) {
    // send data via zmq                
    data = {
      gattid:sensorID,
    ...
    return sensor.send(JSON.stringify(data))
}

// bindAsync = async function() {
//   try {
//     await notify.bind(config.zmqSockets.broker.xsub)
...
//For sensor data input
app.listen(port, () => {            //server starts listening for any attempts from a client to connect at port: {port}
    console.log(`Now listening on port ${port}`); 
});

app.get('/', (req, res) => {
  res.send('The pulse is running')
});
...
app.post("/sensor", (req, res, next) => {
    ...
    const data=req.body;
	sendSensorData(data.air_quality,data.humidity,data.temperature,data.light,data.CO2,data.SensorUUID);
	res.status(200);
